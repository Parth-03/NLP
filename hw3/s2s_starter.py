    ...
    def beam_search(self, seq, beam_size=5):
        ...
        for idx in range(max_len):
            ...
            for b in beams:
                ...
                pred_dist, hn = self.single_decoder_step(prev_char, prev_h)
                _, top_indices = torch.sort(-pred_dist)
                for i in top_indices:
                    candidate = [seq+ [i], curr_prob * pred_dist[i], hn]
                    beam_candidates.append(candidate)
            
            top_k = sorted(beam_candidates, key=lambda tup:tup[1])
            beams.append(top_k[:k])
