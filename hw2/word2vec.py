...
def compute_pos_prob(t_v, c_v):
    #print (sigmoid(np.dot(t_v,c_v)))
    return sigmoid(np.dot(t_v,c_v))


# P(- | t, n_i)c
def compute_neg_prob(t_v, n_v):
    #print (1-sigmoid(np.dot(t_v,n_v)))
    return 1-sigmoid(np.dot(t_v,n_v))
#1/(1+np.exp(-(t_v*n_v))))
...
def compute_obj_and_grad(t, c, W, C, sampling_dist, do_grad_check=False):
    ...
    for n_i in ns:
        ...
        dW[t] += (neg_prob-1)*n_v # IMPLEMENT ME!
        dC[n_i] += (neg_prob-1)*t_v # IMPLEMENT ME!

    return L, dW, dC

def sampling_dist_freq(s_d):
    c= Counter(s_d)
    keys = list(c)
